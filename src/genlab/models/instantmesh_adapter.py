...
class InstantMeshAdapter(Base3DGenModel):
    ...
    def _build_output_mesh_path(
        ...
        return out_dir / f"{input_stem}_instantmesh.obj"
    ...
    def generate(
        ...
        repo_path = Path(model_cfg.get("repo_path", "external/InstantMesh")).resolve()
        if not repo_path.exists():
            raise FileNotFoundError(
                "[InstantMesh][real-mode] External repository missing: "
                f"{repo_path}. Run: bash scripts/setup_external_repos.sh"
            )

        if not input_image:
            raise ValueError(
                "[InstantMesh][real-mode] input_image is required. "
                "Provide --input <image_path> or set input_image in config."
            )
        input_image_path = Path(input_image).resolve()
        if not input_image_path.exists():
            raise FileNotFoundError(
                "[InstantMesh][real-mode] input_image was not found: "
                f"{input_image_path}"
            )
        ...
        if not command_template:
            raise ValueError(
                "[InstantMesh][real-mode] Missing models.instantmesh.inference.command in config."
            )
        ...
        generated_mesh_cfg = inference_cfg.get(
            "generated_mesh",
            "{output_dir}/instant-mesh-large/meshes/{input_stem}.obj",
        )
        ...
        try:
            ...
            raise RuntimeError(
                "[InstantMesh][real-mode] Command failed.\n"
                f"Command: {cmd}\n"
                f"Exit code: {exc.returncode}\n"
                f"Stderr (tail): {stderr_tail}\n"
                "Please verify InstantMesh dependencies and command flags in config."
            ) from exc

        if not generated_mesh.exists():
            raise FileNotFoundError(
                "[InstantMesh][real-mode] Command finished but generated mesh was not found: "
                f"{generated_mesh}. Check models.instantmesh.inference.generated_mesh and command."
            )

        if generated_mesh.resolve() != out_mesh_abs:
            shutil.copy2(generated_mesh, out_mesh_abs)
            log_step(
                "[InstantMesh][real-mode] Copied generated mesh to pipeline output path: "
                f"{out_mesh_abs}"
            )
